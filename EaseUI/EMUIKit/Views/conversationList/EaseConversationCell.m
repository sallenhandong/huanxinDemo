...
- (void)setModel:(id<IConversationModel>)model
{
    ...
    if (_model.conversation.type == 0) {
        
        
        //从服务器获取
        self.avatarView.imageView.image = [UIImage imageNamed:@"head1"];
         self.titleLabel.text = @"单聊会话";
        
    }
    if (_model.conversation.type == 1) {
        EMGroup *group =  [[EMClient sharedClient].groupManager fetchGroupInfo:_model.conversation.conversationId includeMembersList:YES error:nil];
        
        self.titleLabel.text = group.subject;
        
        self.avatarView.imageView.image = [UIImage imageNamed:@"head2"];
